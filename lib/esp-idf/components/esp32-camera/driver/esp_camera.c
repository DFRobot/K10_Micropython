...
static esp_err_t camera_probe(const camera_config_t *config, camera_model_t *out_camera_model)
{
    ...
    *out_camera_model = CAMERA_NONE;
    ...
    if (config->pin_pwdn >= 0) {
        ESP_EARLY_LOGI(TAG, "Resetting camera by power down line");
    ...
    if (config->pin_reset >= 0) {
        ESP_EARLY_LOGI(TAG, "Resetting camera");
        gpio_config_t conf = { 0 };
    ...
    ESP_EARLY_LOGI(TAG, "Searching for camera address");
    vTaskDelay(10 / portTICK_PERIOD_MS);
    ...
    ESP_EARLY_LOGI(TAG, "Detected camera at address=0x%02x", slv_addr);
        ...
        if (g_sensors[i].detect(slv_addr, id)) {
            ...
            if (NULL != info) {
                *out_camera_model = info->model;
                ESP_EARLY_LOGI(TAG, "Detected %s camera", info->name);
    ...
    ESP_EARLY_LOGI(TAG, "Camera PID=0x%02x VER=0x%02x MIDL=0x%02x MIDH=0x%02x",
