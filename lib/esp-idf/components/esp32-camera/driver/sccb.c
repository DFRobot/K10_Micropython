...
{
    ESP_EARLY_LOGI(TAG, "pin_sda %d pin_scl %d", pin_sda, pin_scl);
    i2c_config_t conf;
    esp_err_t ret;
    ...
    ESP_EARLY_LOGI(TAG, "sccb_i2c_port=%d", sccb_i2c_port);
...
uint8_t SCCB_Probe(void)
{
    ...
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, ( 0xff << 1 ) | WRITE_BIT, ACK_CHECK_EN);
    i2c_master_stop(cmd);
    esp_err_t ret = i2c_master_cmd_begin(SCCB_I2C_PORT_DEFAULT, cmd, 1000 / portTICK_RATE_MS);
    i2c_cmd_link_delete(cmd);
    //for (size_t i = 0; i < CAMERA_MODEL_MAX; i++) {
    for (size_t i = 1; i < 3; i++) {
        ...
        slave_addr = 0x3c;//camera_sensor[i].sccb_addr;
        i2c_cmd_handle_t cmd = i2c_cmd_link_create();
        i2c_master_start(cmd);
        i2c_master_write_byte(cmd, ( slave_addr << 1 ) | WRITE_BIT, ACK_CHECK_EN);
        i2c_master_stop(cmd);
        ...
        i2c_cmd_link_delete(cmd);
        if( ret == ESP_OK) {
            ESP_EARLY_LOGI(TAG, "Searching for camera address %d",slave_addr);
            return slave_addr;
        }
    }
    ESP_EARLY_LOGI(TAG,"SCCB_Probe");
    return 0;
}
