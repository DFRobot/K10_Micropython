...
static esp_err_t cam_dma_config(const camera_config_t *config)
{
    ...
    ESP_EARLY_LOGI(TAG, "buffer_size: %d, half_buffer_size: %d, node_buffer_size: %d, node_cnt: %d, total_cnt: %d",
    ...
    for (int x = 0; x < cam_obj->frame_cnt; x++) {
        ...
        ESP_EARLY_LOGI(TAG, "Allocating %d Byte frame buffer in %s", alloc_size, _caps & MALLOC_CAP_SPIRAM ? "PSRAM" : "OnBoard RAM");
        ...
        if (cam_obj->psram_mode) {
            ...
            ESP_EARLY_LOGI(TAG, "Frame[%d]: Offset: %u, Addr: 0x%08X", x, cam_obj->frames[x].fb_offset, (unsigned) cam_obj->frames[x].fb.buf);
...
esp_err_t cam_config(const camera_config_t *config, framesize_t frame_size, uint16_t sensor_pid)
{
    ...
    if(ret != ESP_OK){
        ESP_EARLY_LOGI(TAG, "cam_dma_config failed");
    }
    ...
    xTaskCreatePinnedToCore(cam_task, "cam_task", 4 * 1024, NULL, configMAX_PRIORITIES - 2, &cam_obj->task_handle, 0);
#elif CONFIG_CAMERA_CORE1
    xTaskCreatePinnedToCore(cam_task, "cam_task", 4 * 1024, NULL, configMAX_PRIORITIES - 2, &cam_obj->task_handle, 1);
#else
    xTaskCreate(cam_task, "cam_task", 4 * 1024, NULL, configMAX_PRIORITIES - 2, &cam_obj->task_handle);
#endif
